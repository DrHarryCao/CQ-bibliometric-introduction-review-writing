...
from common import load_jsonl, read_csv_optional, read_json, resolve_artifact, utc_stamp, write_json, write_jsonl
...
def validate_gaps(root: Path) -> dict[str, Any]:
    ...
    evidence_records = load_jsonl(root / "02_corpus/corpus.jsonl")
    evidence_records.extend(load_jsonl(root / "02_corpus/supplemental_writing_evidence.jsonl"))
    corpus_ids = {x.get("record_id") for x in evidence_records}
    errors, warnings = [], []
    normalized = []
    for raw in ledger:
...
def validate_design(root: Path) -> dict[str, Any]:
    ...
    gap_rows = load_jsonl(root / "05_evidence/gaps/gap_ledger.jsonl")
    selected_gaps = [x for x in gap_rows if x.get("status") in {"validated", "opportunity"}]
    recommended_gap_ids = {str(x.get("gap_id") or "") for x in rows}
    for gap in selected_gaps:
        gap_id = str(gap.get("gap_id") or "")
        if not gap.get("research_questions"):
            warnings.append(f"{gap_id}未映射到可检验研究问题")
        if not gap.get("design_requirements"):
            warnings.append(f"{gap_id}未说明修复该缺口所需的识别性设计")
        if gap_id and gap_id not in recommended_gap_ids:
            warnings.append(f"{gap_id}尚无问题—设计—方法映射")
    model_package = resolve_artifact(root, "theory_model_package")
    if model_package and model_package.exists():
        model_text = model_package.read_text(encoding="utf-8", errors="replace")
        if "缺口—问题—设计—贡献闭环" not in model_text:
            warnings.append("理论模型包缺少缺口—问题—设计—贡献闭环")
        for gap in selected_gaps:
            gap_id = str(gap.get("gap_id") or "")
            if gap_id and gap_id not in model_text:
                warnings.append(f"理论模型包未回指缺口{gap_id}")
