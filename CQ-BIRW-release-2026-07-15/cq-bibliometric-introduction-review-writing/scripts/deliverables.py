...
from common import file_sha256, load_jsonl, read_json
from evidence import cited_records, manuscript_path, punctuation_errors, split_embedded_references, sync_references
...
def clean_audit_markers(text: str) -> str:
    ...
    # Preserve paragraph separators when a role marker starts the next
    # paragraph.  ``\s*`` also matches newlines and previously collapsed an
    # audited 10-paragraph introduction into one HTML paragraph.
    text = re.sub(r"[ \t]*\[role:[a-z-]+\]", "", text, flags=re.I)
    token = r"(?:R[0-9a-f]{14}|C\d{4}|SCTX-\d{3,})"
    text = re.sub(r"[ \t]*\\?\[\s*C\d{4}\s*[–—-]\s*C\d{4}\s*\\?\]", "", text, flags=re.I)
    # Remove the complete legacy audit container before removing individual IDs;
    # otherwise strings such as “（R…，page:1）” turn into “（，）”.
    text = re.sub(r"[\(（][ \t]*(?:R[0-9a-f]{14}|C\d{4})(?:[ \t]*[,;，；][ \t]*(?:R[0-9a-f]{14}|C\d{4}|page:\d+|paragraph:\d+))*[ \t]*[\)）]", "", text, flags=re.I)
    text = re.sub(rf"[ \t]*\\?\[(?:cite:)?[ \t]*{token}(?:[ \t]*[,;，；][ \t]*{token})*[ \t]*\\?\]", "", text, flags=re.I)
    text = re.sub(r"[ \t]*R[0-9a-f]{14}\b", "", text)
    text = re.sub(r"[ \t]*SCTX-\d{3,}\b", "", text)
    text = re.sub(r"[ \t]*(?:page|paragraph):\d+\b", "", text)
...
function cqParagraph(el,reference){const tag=el.tagName.toLowerCase(),style=tag==='h1'?'Title':tag==='h2'?'Heading1':tag==='h3'?'Heading2':reference?'Reference':'Normal';const text=cqXml(el.innerText.trim());if(!text)return '';return `<w:p><w:pPr><w:pStyle w:val="${style}"/></w:pPr><w:r><w:rPr><w:rFonts w:ascii="Times New Roman" w:hAnsi="Times New Roman" w:eastAsia="SimSun"/><w:lang w:val="en-US" w:eastAsia="zh-CN"/></w:rPr><w:t xml:space="preserve">${text}</w:t></w:r></w:p>`;}
function exportWord(){const article=document.querySelector('article'),nodes=[...article.querySelectorAll('h1,h2,h3,p,li')];let reference=false,paras='';for(const node of nodes){if(/^h[1-3]$/i.test(node.tagName)&&/参考文献|references/i.test(node.innerText))reference=true;paras+=cqParagraph(node,reference&&!/^h[1-3]$/i.test(node.tagName));}const copyright='JWC💗XQ@Rednote drharry';const files={
...
def export_deliverables(root: Path, document: str, draft: bool = False, variant: str = "evidence-aware") -> dict[str, Any]:
    if variant == "both":
        variants = ["evidence-aware"] + (["publication"] if manuscript_path(root, document, "publication").exists() else [])
        return {"document": document, "variant": "both", "deliverables": {item: export_deliverables(root, document, draft, item) for item in variants}}
    if variant not in {"evidence-aware", "publication"}: raise ValueError("variant must be evidence-aware, publication, or both")
    source = manuscript_path(root, document, variant)
    base = f"{document}_publication" if variant == "publication" else document
    audit_name = f"{base}_audit.md"; clean_name = f"{base}.md"
    title = ("系统综述" if document == "review" else "SSCI漏斗型绪论") + ("（投稿正文版）" if variant == "publication" else "（证据提示版）")
    ...
    if not draft:
        ...
        if not review_source.exists() or approval.get("status") != "approved" or approval.get("source_sha256") != file_sha256(review_source):
            raise RuntimeError("最终交付已阻止：综述尚未获得当前版本的用户确认。请先完成HTML草稿审阅和document-approval。")
        if variant == "publication":
            validation = read_json(root / f"07_logs/publication_validation_{document}.json", {})
            if not validation.get("valid") or validation.get("source_sha256") != file_sha256(source):
                raise RuntimeError("投稿版交付已阻止：投稿验证未通过或已过期。请运行 build-publication --phase validate。")
        else:
            validation = read_json(root / "07_logs/validation_report.json", {})
            if not validation.get("valid"):
                raise RuntimeError("最终交付已阻止：最新总体验证未通过。请先运行 validate。")
            if (validation.get("source_hashes") or {}).get(document) != file_sha256(source):
                raise RuntimeError("最终交付已阻止：正文在最近验证后发生变更，请重新同步引用、逐句审计并运行 validate。")
    ...
    clean_body, _ = split_embedded_references(clean)
    if document == "introduction":
        paragraphs = [p for p in re.split(r"\n\s*\n", clean_body) if p.strip() and not p.lstrip().startswith("#")]
        rendered_body, _ = markdown_body(clean_body)
        if not 8 <= len(paragraphs) <= 12 or rendered_body.count("<p>") != len(paragraphs):
            raise RuntimeError(f"绪论HTML正文段落验证失败：Markdown={len(paragraphs)}，HTML={rendered_body.count('<p>')}；参考文献不计入。")
    out = root / "06_review" / ("previews" if draft else "deliverables"); out.mkdir(parents=True, exist_ok=True)
    if draft:
        audit_name = f"{base}_audit_draft.md"; clean_name = f"{base}_draft.md"; title += "（未验证草稿预览）"
    ...
    for stale_docx in out.glob("*.docx"):
        stale_docx.unlink()
    html_path = out / (f"{base}_draft.html" if draft else f"{base}.html"); html_path.write_text(build_html(title, clean), encoding="utf-8")
    ledger = load_jsonl(root / "05_evidence/claim_ledger.jsonl"); ids = cited_records(body, ledger)
    record_rows = load_jsonl(root / "02_corpus/corpus.jsonl")
    record_rows.extend(load_jsonl(root / "02_corpus/supplemental_writing_evidence.jsonl"))
    records = {r["record_id"]: r for r in record_rows}; missing = sorted(ids - records.keys())
    ...
    social_rows = [row for index, row in enumerate(social_rows, 1) if (row.get("source_id") or f"SCTX-{index:03d}") in explicit_social]
    ris_blocks.extend(social_source_to_ris(row, index) for index, row in enumerate(social_rows, 1))
    ris = out / (f"{base}_references_draft.ris" if draft else f"{base}_references.ris"); ris.write_text("\n\n".join(ris_blocks) + "\n", encoding="utf-8")
    ris_ids = set(re.findall(r"(?m)^ID  - (R[0-9a-f]{14})$", ris.read_text(encoding="utf-8")))
    ris_social = set(re.findall(r"(?m)^ID  - (SCTX-\d{3,})$", ris.read_text(encoding="utf-8")))
    if ris_ids != ids or ris_social != explicit_social:
        raise RuntimeError(f"正文—RIS引用集不一致：records={len(ids)}/{len(ris_ids)}, social={len(explicit_social)}/{len(ris_social)}")
    if draft:
        return {"document": document, "variant": variant, "status": "draft-preview-not-validated", "audit_md": str(out / audit_name), "clean_md": str(out / clean_name), "html": str(html_path), "ris": str(ris), "cited_records": len(ids), "social_sources": len(social_rows)}
    index = out / "index.html"
    items = []
    labels = {"review": "系统综述·证据提示版", "review_publication": "系统综述·投稿正文版", "introduction": "绪论·证据提示版", "introduction_publication": "绪论·投稿正文版"}
    for kind in ("review", "review_publication", "introduction", "introduction_publication"):
        if (out / f"{kind}.html").exists(): items.append(f'<li><a href="{kind}.html">{labels[kind]}</a> · HTML内导出Word/PDF · <a href="{kind}_references.ris">RIS</a></li>')
    ...
    return {"document": document, "variant": variant, "audit_md": str(out / audit_name), "clean_md": str(out / clean_name), "html": str(html_path), "word_export": "browser-generated-docx", "ris": str(ris), "cited_records": len(ids), "social_sources": len(social_rows), "index": str(index)}
