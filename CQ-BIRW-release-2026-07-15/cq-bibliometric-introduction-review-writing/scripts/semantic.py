...
def prepare(root: Path, batch_size: int = 12, budget: str = "balanced") -> dict[str, Any]:
    ...
    target_rows = load_jsonl(root / "05_evidence/citation_coverage_targets.jsonl")
    target_ids = {x.get("record_id") for x in target_rows}
    target_order = {x.get("record_id"): index for index, x in enumerate(target_rows)}
    if budget == "balanced" and target_ids: records = [r for r in records if r.get("record_id") in target_ids]
    # Supplemental records are host-approved content evidence.  They expand
    # mechanism/outcome support but never enter NMF or alter the analytic corpus.
    existing_ids = {r.get("record_id") for r in records}
    records.extend(r for r in supplemental if r.get("record_id") not in existing_ids)
    ...
    tasks = []
    for record in sorted(records, key=lambda r: (target_order.get(r["record_id"], 10**9), topic.get(r["record_id"], 0), r.get("year") or 0)):
        ...
        target_meta = next((x for x in target_rows if x.get("record_id") == record["record_id"]), {})
        tasks.append({"record_id": record["record_id"], "topic_id": int(topic.get(record["record_id"], 0) or 0), "title": record.get("title", ""), "year": record.get("year"), "evidence_level": level, "evidence_tier": target_meta.get("evidence_tier", record.get("evidence_tier", "")), "selection_reason": target_meta.get("selection_reason", "exhaustive"), "source_hash": source_hash, "source_path": full or f"05_evidence/cards/{record['record_id']}.md", "abstract": record.get("abstract", "") if not full else "", "output_path": f"05_evidence/semantic/extractions/{record['record_id']}.json", "template": _template(record, level, source_hash)})
    ...
    write_json(base / "selection_manifest.json", {"protocol_version": PROTOCOL_VERSION, "budget": budget, "selected_record_ids": [record["record_id"] for record in records], "selected_records": len(records)})
...
def validate_semantic(root: Path) -> dict[str, Any]:
    ...
    corpus.extend(load_jsonl(root / "02_corpus/supplemental_writing_evidence.jsonl"))
    records = {r["record_id"]: r for r in corpus}; errors, warnings = [], []
