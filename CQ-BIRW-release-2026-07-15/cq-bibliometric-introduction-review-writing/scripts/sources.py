...
class CachedClient:
    def __init__(self, cache_dir: Path, timeout: int = 45, retries: int = 5):
        ...
        self.session.headers.update({"User-Agent": "cq-bibliometric-introduction-review-writing/8.0 (scholarly research workflow)"})
...
def enrich_crossref(record: dict[str, Any], client: CachedClient, email: str = "") -> dict[str, Any]:
    ...
    try:
        ...
        # A DOI can be valid in another registry while Crossref has no matching
        # work. Missing optional enrichment must not abort an otherwise valid
        # OpenAlex search task.
        if "404" in str(exc):
            record.setdefault("metadata_enrichment_notes", []).append({
                "source": "crossref", "status": "not-found", "doi": doi,
            })
            return record
        raise
...
def download_oa_files(root: Path, records: list[dict[str, Any]], limit: int = 100, allow_openalex_content: bool = False) -> dict[str, Any]:
    ...
    session = requests.Session(); session.headers.update({"User-Agent": "cq-bibliometric-introduction-review-writing/8.0"})
