...
def read_csv_optional(path: Path, columns: Iterable[str] | None = None, **kwargs: Any) -> pd.DataFrame:
    """Read an optional UTF-8 CSV without turning an absent analysis into a crash.

    Missing, zero-byte, BOM/whitespace-only, header-only, and pandas-empty files all
    return a frame with the requested schema. Malformed non-empty CSV remains an
    error because silently hiding corrupted evidence would be unsafe.
    """
    ...
    raw = path.read_bytes()
    if not raw.decode("utf-8-sig", errors="ignore").strip():
        return pd.DataFrame(columns=expected)
    try:
        frame = pd.read_csv(path, encoding=kwargs.pop("encoding", "utf-8-sig"), **kwargs).fillna("")
    ...
    if expected:
        frame = frame[expected + [c for c in frame.columns if c not in expected]]
    return frame
...
def canonical_record(data: dict[str, Any], source: str = "unknown") -> dict[str, Any]:
    ...
    record = {
        ...
        "publication_type_normalized": data.get("publication_type_normalized") or "",
        "peer_review_status": data.get("peer_review_status") or "",
        "evidence_tier": data.get("evidence_tier") or "",
        "tier_reason": data.get("tier_reason") or "",
        "claim_use_roles": as_list(data.get("claim_use_roles")),
...
def records_match(a: dict[str, Any], b: dict[str, Any]) -> tuple[bool, str, float]:
    ...
    if ad and bd:
        if ad != bd:
            return False, "different-doi", 0.0
        # A DOI may identify an issue, book, proceedings volume, or chapter and
        # is occasionally copied onto multiple article records.  It is strong
        # evidence, not unconditional identity.
        ...
        if at and bt:
            ...
            aa, ba = (a.get("authors") or [{}])[0], (b.get("authors") or [{}])[0]
            aa = aa.get("name", "") if isinstance(aa, dict) else str(aa); ba = ba.get("name", "") if isinstance(ba, dict) else str(ba)
            author_ok = not aa or not ba or SequenceMatcher(None, normalize_title(aa), normalize_title(ba)).ratio() >= .65
            if score >= 0.90 and year_ok and author_ok:
                return True, "doi-title-confirmed", score
            if score <= 0.65 or not year_ok or (score < .98 and not author_ok):
                return False, "doi-scope-conflict", score
            return False, "doi-ambiguous", score
        # Missing titles require a second stable identifier; otherwise retain
        # both records for review instead of silently collapsing them.
        for key in ("openalex", "pmid", "pmcid", "wos"):
            if ai.get(key) and bi.get(key) and str(ai[key]).rsplit("/", 1)[-1] == str(bi[key]).rsplit("/", 1)[-1]:
                return True, f"doi-{key}-confirmed", 1.0
        return False, "doi-missing-title-review", 0.5
...
def deduplicate(records: Iterable[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for raw in records:
        ...
        # Compare a repeated DOI against every retained member of that DOI
        # group.  A conflicting pair must not inherit an earlier pair's merge.
        candidates = [i for i, item in enumerate(unique) if doi and normalize_doi((item.get("ids") or {}).get("doi")) == doi] if doi and doi in doi_index else range(len(unique))
        matched = None
        ...
        if matched is None:
            if doi and doi in doi_index:
                ...
                for idx in candidates:
                    ok, method, score = records_match(unique[idx], rec)
                    if not ok and method.startswith("doi-"):
                        audit.append({"kept_id": unique[idx]["record_id"], "retained_id": rec["record_id"], "method": method, "score": round(score, 4), "action": "retained-separately", "doi": doi, "conflicts": []})
                # DOI-derived IDs collide for scope conflicts. Preserve the DOI
                # as metadata but create a title-scoped stable local identity.
                if any(item.get("record_id") == rec.get("record_id") for item in unique):
                    seed = f"doi-scope:{doi}:{normalize_title(rec.get('title'))}:{rec.get('year') or ''}"
                    rec["record_id"] = "R" + hashlib.sha1(seed.encode("utf-8")).hexdigest()[:14]
                for item in audit[audit_start:]: item["retained_id"] = rec["record_id"]
            unique.append(rec)
...
def resolve_artifact(root: Path, kind: str) -> Path:
    """Resolve current and historical artifact locations consistently."""
    candidates = {
        "theory_model_package": [root / "05_evidence/design/theory_model_package.md", root / "06_review/theory_model_package.md"],
        "focus_plan": [root / "00_plan/focus_plan.json"],
    }.get(kind)
    if not candidates:
...
def update_manifest(root: Path, event: str, details: dict[str, Any] | None = None) -> None:
    ...
    manifest["skill_schema_version"] = 12
    ...
    if previous_schema < 8 and not any(e.get("event") == "schema-migrated-v8" for e in manifest.get("events", [])):
        manifest.setdefault("events", []).append({"at": utc_stamp(), "event": "schema-migrated-v8", "details": {"from": previous_schema, "to": 8, "data_files_modified": False, "new_optional_artifacts": ["preprocessing_audit", "reconciliation_tasks", "effect_sizes", "prose_claim_ledger", "reference_audit"]}})
    if previous_schema < 9 and not any(e.get("event") == "schema-migrated-v9" for e in manifest.get("events", [])):
        manifest.setdefault("events", []).append({"at": utc_stamp(), "event": "schema-migrated-v9", "details": {"from": previous_schema, "to": 9, "data_files_modified": False, "new_optional_artifacts": ["citation_repair_queue", "review_approval", "nmf_structure_gate"]}})
    if previous_schema < 10 and not any(e.get("event") == "schema-migrated-v10" for e in manifest.get("events", [])):
        manifest.setdefault("events", []).append({"at": utc_stamp(), "event": "schema-migrated-v10", "details": {"from": previous_schema, "to": 10, "data_files_modified": False, "new_optional_artifacts": ["publication_variants", "publication_rewrite_queue", "prose_atomic_claim_ledger", "citation_attribution_queue"]}})
    if previous_schema < 11 and not any(e.get("event") == "schema-migrated-v11" for e in manifest.get("events", [])):
        manifest.setdefault("events", []).append({"at": utc_stamp(), "event": "schema-migrated-v11", "details": {"from": previous_schema, "to": 11, "data_files_modified": False, "new_optional_artifacts": ["external_theory_library", "theory_fit_ledger", "theory_degradation_log"]}})
    if previous_schema < 12 and not any(e.get("event") == "schema-migrated-v12" for e in manifest.get("events", [])):
        manifest.setdefault("events", []).append({"at": utc_stamp(), "event": "schema-migrated-v12", "details": {"from": previous_schema, "to": 12, "data_files_modified": False, "new_optional_artifacts": ["focus_plan", "evidence_source_hierarchy", "citation_selection_audit"]}})
    manifest.setdefault("theory_support_mode", "unselected")
    manifest.setdefault("theory_support_non_blocking", True)
