...
from common import canonical_record, deduplicate, ensure_task, file_sha256, load_jsonl, normalize_doi, normalize_title, read_csv_optional, read_json, update_manifest, utc_stamp, write_json, write_jsonl
...
from focus import focus_plan_template, focus_records, validate_focus_plan
from derivation import derive_task, inherited_manifest
...
from writing_evidence_expansion import compile_selection as compile_writing_evidence_selection, search_expansion as search_writing_evidence_expansion
...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    update_manifest(root, "init", {"title": args.title, "skill_schema_version": 12, "path_preflight": issues})
...
def cmd_expand_writing_evidence(args: argparse.Namespace) -> int:
    root = root_of(args)
    report = search_writing_evidence_expansion(root, args.refresh) if args.phase == "search" else compile_writing_evidence_selection(root)
    update_manifest(root, f"expand-writing-evidence-{args.phase}", report)
...
def cmd_corpus_policy(args: argparse.Namespace) -> int:
    if args.mode == "focused":
        ...
        source_root = root_of(args); focus_spec = source_root / "00_plan/focus_plan.json"
        if not focus_spec.exists() or not validate_focus_plan(read_json(focus_spec, {})).get("valid"):
            if not focus_spec.exists(): cmd_focus_plan(argparse.Namespace(task=args.task, phase="prepare"))
            manifest = read_json(source_root / "manifest.json", {}); manifest["corpus_mode"] = "focused-pending-plan"; manifest["focused_output_task"] = str(Path(args.output_task).expanduser().resolve()); write_json(source_root / "manifest.json", manifest)
            report = {"status": "focus-plan-required", "plan": str(focus_spec), "output_task": manifest["focused_output_task"], "user_prompt": "请审阅并批准任务专用聚焦计划；宿主须先补全当前领域的中英文规则。"}
            print(json.dumps(report, ensure_ascii=False, indent=2)); return 0
        return cmd_focus(argparse.Namespace(task=args.task, output_task=args.output_task, spec=None))
...
def cmd_focus_plan(args: argparse.Namespace) -> int:
    root = root_of(args); path = root / "00_plan/focus_plan.json"
    if args.phase == "prepare":
        if getattr(args, "migrate_legacy_profile", None):
            profiles = read_json(SCRIPT_DIR.parent / "assets/legacy_focus_profiles.json", {})
            plan = profiles.get(args.migrate_legacy_profile)
            if not plan: raise RuntimeError("未知旧聚焦配置")
            write_json(path, plan)
        elif not path.exists(): write_json(path, focus_plan_template(read_json(root / "00_plan/search_plan.json", {})))
        plan = read_json(path, {})
        lines = ["# 任务专用聚焦计划", "", f"- 主题：{plan.get('topic_label') or '待补全'}", f"- 状态：{plan.get('status')}", "", "> 本文件不含任何内置领域词表。请由宿主依据本任务研究问题补全中英文核心组、相邻机制与排除概念，再展示给用户确认。", "", "## 核心概念组", ""]
        for group in plan.get("core_groups") or []: lines.append(f"- {group.get('id')}: {group.get('label')} — {', '.join(group.get('terms') or []) or '待补全'}")
        (root / "00_plan/focus_plan.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
        report = {"status": "prepared", "plan": str(path), "requires_user_confirmation": True, "domain_hardcoding": False}
    else:
        report = validate_focus_plan(read_json(path, {}))
        write_json(root / "07_logs/focus_plan_validation.json", report)
    update_manifest(root, f"focus-plan-{args.phase}", report); print(json.dumps(report, ensure_ascii=False, indent=2)); return 0 if report.get("valid", True) else 1
...
def cmd_focus(args: argparse.Namespace) -> int:
    ...
    spec = Path(args.spec).expanduser().resolve() if getattr(args, "spec", None) else source_root / "00_plan/focus_plan.json"
    if not spec.exists():
        raise RuntimeError("缺少任务专用 focus_plan.json。请先运行 focus-plan --phase prepare，由宿主补全规则并经用户确认后再执行聚焦。")
    plan = read_json(spec, {})
    validation = validate_focus_plan(plan)
    if not validation["valid"]: raise RuntimeError("聚焦计划不可执行：" + "；".join(validation["errors"]))
    core, theory, needs_review, excluded, report = focus_records(records, plan)
    if len(core) < 4: raise RuntimeError(f"聚焦核心仅得到 {len(core)} 篇，已停止创建派生任务；请调整筛选规则。")
    output_root, _ = derive_task(source_root, output_root, "focused-corpus", {"corpus_mode": "focused"})
    ...
    write_jsonl(output_root / "02_corpus/focus_needs_review.jsonl", needs_review)
    write_jsonl(output_root / "02_corpus/excluded_focus_records.jsonl", excluded)
    screening = ["# 聚焦核心语料筛选", "", f"- 任务主题：{plan.get('topic_label')}", f"- 原始语料：{len(records)}", f"- 直接核心：{len(core)}", f"- 理论补充池：{len(theory)}（不进入主题模型）", f"- 待确认：{len(needs_review)}", f"- 明确排除：{len(excluded)}", "", "> 聚焦规则完全来自本任务已批准的 focus_plan.json；核心代码不含直播电商、LARP或其他领域默认词表。", "", "## 分类原因统计", ""]
    screening.extend(f"- {reason}: {count}" for reason, count in sorted(report["reason_counts"].items(), key=lambda x: x[1], reverse=True))
...
def cmd_status(args: argparse.Namespace) -> int:
    root = root_of(args)
    if int(read_json(root / "manifest.json", {}).get("skill_schema_version", 1)) < 12:
        update_manifest(root, "schema-migration-on-status", {"data_files_modified": False})
    corpus = load_jsonl(root / "02_corpus/corpus.jsonl")
    ...
    semantic_selection = read_json(root / "05_evidence/semantic/selection_manifest.json", {})
    planned_semantic = int(semantic_selection.get("selected_records") or 0) or len({x.get("record_id") for x in batch_tasks if x.get("record_id")}) or len(load_jsonl(root / "05_evidence/citation_coverage_targets.jsonl"))
    ...
    manifest, inherited_fields = inherited_manifest(root, read_json(root / "manifest.json", {}))
    ...
    focus_plan = read_json(root / "00_plan/focus_plan.json", {})
    hierarchy = read_csv_optional(root / "05_evidence/evidence_source_hierarchy.csv", ["evidence_tier"])
    status.update({"inherited_parent_fields": inherited_fields, "focus_plan_status": "validated" if validate_focus_plan(focus_plan).get("valid") else "needs-review" if focus_plan else "missing", "focus_needs_review": len(load_jsonl(root / "02_corpus/focus_needs_review.jsonl")), "evidence_tiers": {str(k): int(v) for k, v in hierarchy["evidence_tier"].value_counts().items()} if not hierarchy.empty else {}, "tier_fallbacks": int(read_json(root / "05_evidence/evidence_build_report.json", {}).get("tier_fallbacks", 0))})
    ...
    writing_quality = load_jsonl(root / "05_evidence/writing_quality_repair_queue.jsonl")
    status["writing_quality_repair_queue"] = {
        "items": len(writing_quality),
        "by_document": {document: sum(x.get("document") == document for x in writing_quality) for document in ("review", "introduction")},
        "blocking": False,
    }
    ...
    elif status["corpus_mode"] == "focused-pending-plan" and status["focus_plan_status"] != "validated": next_step = "补全并展示 focus_plan.json，批准后运行 focus-plan --phase validate 与 focus"; checkpoint = "focus-plan-approval"; requires_input = True; prompt = "请回复“确认聚焦语料”，或列出需要调整的纳入/排除概念。"; replies = ["确认聚焦语料", "调整：……"]
    elif status["corpus_mode"] == "focused-pending-plan": next_step = f"自动运行 focus --task TASK --output-task {manifest.get('focused_output_task', 'FOCUSED_TASK')}，随后切换到派生任务继续分析"
...
def parser() -> argparse.ArgumentParser:
    ...
    focus_plan = sub.add_parser("focus-plan", help="准备或验证当前任务专用、领域无关的聚焦规则"); focus_plan.add_argument("--task", required=True); focus_plan.add_argument("--phase", required=True, choices=["prepare", "validate"]); focus_plan.add_argument("--migrate-legacy-profile", choices=["livestream-commerce", "larp"], help="仅显式迁移旧任务；生成后仍须审阅和确认"); focus_plan.set_defaults(func=cmd_focus_plan)
    focus = sub.add_parser("focus"); focus.add_argument("--task", required=True, help="包含完整语料的原任务目录"); focus.add_argument("--output-task", required=True, help="新建的聚焦任务目录"); focus.add_argument("--spec", help="已批准的 focus_plan.json；默认读取任务 00_plan/focus_plan.json"); focus.set_defaults(func=cmd_focus)
    ...
    we = sub.add_parser("expand-writing-evidence"); we.add_argument("--task", required=True); we.add_argument("--phase", required=True, choices=["search", "compile"]); we.add_argument("--refresh", action="store_true"); we.set_defaults(func=cmd_expand_writing_evidence)
