...
PROPOSITION = re.compile(
    r"本研究(?:提出|认为|拟|将)|有待(?:检验|验证)|需要(?:检验|验证)|"
    r"仍需.{0,24}(?:检验|验证)|待检验命题|可检验(?:模型|命题)|理论命题|"
    r"候选(?:中介|机制|解释)|可能|尚待|尚未|未(?:经|获|被|能|建立|解释)|"
    r"不构成|不是已经",
    re.I,
)
...
def prepare_publication(root: Path, document: str) -> dict[str, Any]:
    ...
    previous_prepare = read_json(root / f"07_logs/publication_prepare_{document}.json", {})
    # Rebase the publication audit draft whenever the evidence-aware source has
    # changed.  Without this hash check, resumed tasks silently keep an obsolete
    # publication version after the review is revised or new evidence is added.
    source_changed = previous_prepare.get("source_sha256") != source_sha256
    if not target.exists() or source_changed:
    ...
    report = {"phase": "prepare", "document": document, "status": "prepared", "source": str(source), "source_sha256": source_sha256, "publication_audit": str(target), "rewrite_queue": str(queue_path), "queue_items": len(queue), "source_preserved": not source_changed, "publication_rebased": source_changed}
