...
def write_brief(root: Path, document: str, gaps: list[str], skipped: bool, model_requested: bool, method_preferences: list[str] | None = None, method_constraints: list[str] | None = None) -> dict[str, Any]:
    if document == "introduction" and not (root / "06_review/review_draft.md").exists():
        raise RuntimeError("系统综述尚未完成，不能建立绪论写作简报。")
    dimensions = ["理论解释", "竞争机制", "测量与构念", "因果识别", "证据一致性", "边界与外部效度", "时间过程", "样本独立性", "实践干预"]
    data = {"document": document, "status": "completed", "user_gap_directions": gaps, "user_skipped": skipped, "default_gap_dimensions": dimensions if skipped else [], "gap_policy": "A-explanatory_then_B-evidential_then_C-opportunity", "method_selection_mode": "corpus-evidence-plus-general-method-library", "method_preferences": method_preferences or [], "method_constraints": method_constraints or [], "model_package_requested": model_requested, "updated_at": utc_stamp()}
    stem = "review_brief" if document == "review" else "introduction_brief"
    write_json(root / f"06_review/{stem}.json", data)
    lines = [f"# {document} writing brief", "", f"- user_skipped: {str(skipped).lower()}", f"- model_package_requested: {str(model_requested).lower()}", f"- gap_policy: {data['gap_policy']}", f"- method_selection_mode: {data['method_selection_mode']}", f"- method_preferences: {', '.join(data['method_preferences']) or 'none'}", f"- method_constraints: {', '.join(data['method_constraints']) or 'none'}", "", "## 希望突出的缺口方向", ""]
    lines += [f"- {x}" for x in (gaps or dimensions)]
    lines += ["", "## 每个缺口必须回答", "", "- 已有研究解释了什么", "- 理论或证据在哪一点失效", "- 支持失效判断的直接证据和反例", "- 构念、结果、样本、情境、时间和设计是否可比", "- 知识与实践后果", "- 可区分竞争解释的研究问题/预测", "- 修复所需的设计与数据", "- 先选设计，再动态匹配主要、替代和稳健性方法", "", "> 研究较少、换情境或未使用某方法只能生成C级研究机会；没有A/B级缺口时允许定稿但必须明确降级警告。"]
...
def writing_context_manifest(root: Path, budget: str, batch_size: int = 12) -> dict[str, Any]:
    records = load_jsonl(root / "02_corpus/corpus.jsonl")
    supplemental = load_jsonl(root / "02_corpus/supplemental_writing_evidence.jsonl")
    analytic_count = len(records)
    targets = load_jsonl(root / "05_evidence/citation_coverage_targets.jsonl")
    selected = list(records) if budget == "exhaustive" else [r for r in records if r["record_id"] in {x.get("record_id") for x in targets}]
    existing_ids = {record.get("record_id") for record in selected}
    selected.extend(record for record in supplemental if record.get("record_id") not in existing_ids)
    tokens = sum(max(1, len((r.get("title") or "") + " " + (r.get("abstract") or "")) // 4) for r in selected)
    report = {"budget": budget, "corpus_records": analytic_count, "supplemental_writing_records": len(supplemental), "semantic_records": len(selected), "estimated_input_tokens": tokens, "estimated_batches": math.ceil(len(selected) / max(batch_size, 1)), "selection": "analytic-corpus+host-approved-supplement" if supplemental else ("all" if budget == "exhaustive" else "citation-quota+fulltext+counterevidence"), "generated_at": utc_stamp()}
