...
- For focused mode, first run `focus-plan --task <all-task> --phase prepare`. Build bilingual core concept groups, required combinations, adjacent mechanism terms, explicit exclusions and a low-confidence rule from the current task only; show this plan to the user, set it approved, and run `focus-plan --phase validate`. Never reuse another field's vocabulary or silently default to livestream commerce, LARP, or any other domain.
- Only when migrating a documented historical task may the host explicitly use `--migrate-legacy-profile livestream-commerce|larp`; the generated example remains unapproved and must be reviewed against the current question. Never use this option for a new task.
- Then create the derivative with `focus --task <all-task> --output-task <focused-task> --spec <focus_plan.json>`. Preserve the all-record task. Analyze only `core`; retain `theory-supplement`, `needs-review`, and explicit `excluded` records in separate auditable files. A derived task must inherit stable acquisition/resume choices but never credentials, transient errors, or stale writing hashes.
...
- Run `build-evidence`. Inspect `evidence_source_hierarchy` and `citation_selection_audit`: semantic relevance and design fit come first, then Tier A→B→C. Tier D remains available for bibliometric auditing but cannot support formal positive claims. Use lower tiers only for documented quota shortage, unique context, emerging evidence, or counterevidence; never let publication type substitute for content quality.
- When the direct focused corpus is too sparse for a defensible mechanism or outcome synthesis, create a task-specific `writing_evidence_expansion_plan.json`, then run `expand-writing-evidence --phase search`. Host-review candidates into `writing_evidence_selection.jsonl` and run `--phase compile`. Keep supplemental records out of NMF and label direct versus adjacent use explicitly; include them in semantic extraction, citation coverage, reference registration, and writing audits.
...
- Read [transferable-writing-patterns.md](references/transferable-writing-patterns.md) before outlining, gap synthesis, theory/model organization, review prose, or introduction prose. Use it only as a writing and reasoning protocol: it is never evidence and may not supply factual claims, citations, theories, or domain findings.
...
- Draft one section at a time into `06_review/sections/`, using visible APA 7 citations plus `[cite:R...,R...]` audit bindings. First identify the document genre, then create a section argument contract and map each smallest verifiable claim to its exact supporting records. Organize explanatory synthesis as theory position → construct comparison → mechanism integration → disagreement/boundary → synthesis, adapting that sequence when the genre requires a different logic. Immediately run `audit-writing --source SECTION --scope SECTION_ID`; resolve both evidence-repair queues before merging and review the non-blocking writing-quality queue. Never leave one large citation cluster to support heterogeneous objects, mechanisms, media, or outcomes, and never copy all paragraph-end citations into every sentence. Every substantive knowledge-topic paragraph must cite related evidence and end with synthesis, qualification, or a transition rather than a literature list.
...
<PYTHON> <PIPELINE> expand-writing-evidence --task TASK --phase search|compile
